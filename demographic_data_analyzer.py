...
def calculate_demographic_data(print_data=True):
    ...
    low_salary=df.loc[df["salary"]=="<=50K","native-country"].value_counts()
    high_salary=df.loc[df["salary"]==">50K","native-country"].value_counts()
    total_salary=(low_salary+high_salary).sort_values(ascending=False)

    earning_percentages=(high_salary/total_salary*100).sort_values(ascending=False)

    highest_earning_country = earning_percentages.index[0]
